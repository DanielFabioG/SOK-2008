...
$$L^D = \frac{\mathrm{E}}{\alpha} - \frac{1}{\alpha}w$$

$$L^S = -\frac{T}{\beta}+\frac{1}{\beta}w$$

Vi setter disse lik hverandre for å kunne løse for w for å finne lønningen

$$\frac{\mathrm{E}}{\alpha} - \frac{1}{\alpha}w = -\frac{T}{\beta}+\frac{1}{\beta}w$$

Og løser for w og da får vi lønnen

$$w = \frac{T\alpha + \mathrm{E}\beta}{\alpha+\beta}$$
...
# Utledning matematisk av minstelonn under fullkomment arbeidsmarked og under monopsoni

# Ettersporsel på arbeid
def L_D(E,alpha,w):
...
# Løser for ettersporsel og finner ettersporselskurven
etterspørsel = sp.solve(sp.Eq(L, L_D(E,alpha,w)), w)

# Definerer ettersporselskurven, bytter om på L og a men har lite betydning
def e_p(E,L,a):
    return E - a*L
...
# Definerer tilbudskurven, bytter litt om po variablene slik at det blir enklere
def t_b(T,b,L):
    return T + b*L 
...
# Definerer numeriske verdier for o plotte
E_num = 250
a = 0.5
x = np.arange(0,550,1)
b = 0.5
...
eurostat %>%
  ...
  labs(y="Minstelønninger", x="Sysselsetting for alder 20-64",
       ...
       title= "Minstelønninger i prosent av gjennomsnittlig bruttoinntekt i 2021",
