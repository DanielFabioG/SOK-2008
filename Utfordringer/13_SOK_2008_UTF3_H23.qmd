...
# Definer l-verdier fra 0.01 til 60
l_values = np.linspace(0.1, l0_num, 500)
```

```{python}
# Situasjon 1: Ingen stønad
venstre1 = (m + l*w)**theta * (l0-l)**(1-theta)
hoyre1 = m**theta * l0**(1-theta)
eq1 = sp.Eq(venstre1, hoyre1)

# Løser ligning 1 for w
sol1 = sp.solve(eq1, w)[0]

# Situasjon 2: Stønad = 100
venstre2 = (m +l*w)**theta * (l0-l)**(1-theta)
hoyre2 = (m+s)**theta * l0**(1-theta)
eq2 = sp.Eq(venstre2, hoyre2)
...
def wages_2(l_values):
    return m_num+s_num + sol_2_num * (l0_num-l_values)
```
...
ax.plot(l_values, wages(l_values), label="Budsjettbettingelse")
#ax.plot(l_values, wages_2(l_values), label="Budsjettbettingelse med stønad")
...
ax.text(x=61,y=100, s="A", fontsize=14)
#ax.text(x=61,y=200,s="B", fontsize=14)
ax.text(x=21,y=320, s="C", fontsize=14)
#ax.text(x=21, y=620, s="D", fontsize=14)
# 
...
Tone har en arbeidsfri inntekt (m ) lik 100 (i ti-tall kroner). Hun kan ikke velge sin arbeidstid fritt. Dersom Tone jobber, må hun jobbe 40 timer per uke. Hun kan maksimalt jobbe 60 timer per uke (l0=60). Dersom Tone ikke tar en jobb, har hun tilgang til stønad.
...
3.1.2 Kun diskusjon rundt årsaker ingen matematikk, resonner rundt spørsmålet.

# Utfordring 3.2.1

Arbeidsmarkedet er sjelden perfekt. Forklar hvilke effekter trygd har på arbeidsmarkedet da arbeidsmarkedet er ufullkomment. Forklar 3 punkter. Forklare konsekvenser. hva som er relevant. bare ord. Trenger ikke matte, figurer osv. 3 effekter de er ute etter. Kan føre til økt arbeidsledighet, lengere tid i arbeidsledighet, økt sysselsetting/redusert sysselsetting. . Job search effect: Unemployment benefits increase the (dynamic) reservationwage ofUB recipients because theymake workersmore choosy when deciding on available job offers, reducing their search intensity and hence increasing the duration oftheir unemployment spells.
